...
public class BloodAxe extends ItemSword implements IHasModel
{
	private double damage;
	private double speed;
	
	public BloodAxe(String name, ToolMaterial material, double speed, double damage) 
	{
		super(material);
	...
	public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
		...
		if(isSelected)
		{
			if(entityIn instanceof EntityPlayer)
			{
				...
				player.addPotionEffect(new PotionEffect(Potion.getPotionById(5), 15*20, 1));
	...
	public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
		// TODO Auto-generated method stub
		attacker.setHealth((float) (attacker.getHealth() + this.getRealDamage(stack)*0.5));
	...
	public float getRealDamage(ItemStack item)
	{
		float dmg = 0;
		for (AttributeModifier modifier : item.getAttributeModifiers(EntityEquipmentSlot.MAINHAND).get(SharedMonsterAttributes.ATTACK_DAMAGE.getName()))
		{
